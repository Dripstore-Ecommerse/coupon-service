...
This microservice manages the coupon catalog and provides information about the available coupons.
...
- **Process management**: advanced couponion process management using [PM2](https://pm2.keymetrics.io)
...
# run docker container in couponion mode
yarn docker:prod
...
In couponion mode, only `info`, `warn`, and `error` logs will be printed to the console.\
It is up to the server (or process manager) to actually read them from the console and store them in log files.\
This app uses pm2 in couponion mode, which is already configured to store the logs in log files.
